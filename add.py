...
def ad():
    ...
    ls=name.get()+'|'+op.get()+'|'+age.get()+'|'+phone.get()+'|'+diseace.get()+'|'+address.get()+'|'+m1+"\n"
...
if __name__=="__main__":
    ...
    label1=Label(root,text="Voter Name:")
    label1.place(x=300,y=120)

    label2=Label(root,text="ID No:")
    ...
    label5=Label(root,text="Consistency:")
    ...
    e5=Entry(root,textvariable=diseace,width=40)
    e5.place(x=420,y=320)

    e6=Entry(root,textvariable=address,width=40)
    e6.place(x=420,y=370)
   
    b4=Button(root,text="Submit",command=add_voter,activebackground="pink",bg="#68BBE3",width=30)
